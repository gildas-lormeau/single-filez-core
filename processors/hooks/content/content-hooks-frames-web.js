...
(globalThis => {
	...
	addEventListener(BLOCK_STORAGE_START_EVENT, () => {
		if (!globalThis._singleFile_localStorage) {
			...
			globalThis.__defineGetter__("localStorage", () => { throw new Error("localStorage temporary blocked by SingleFileZ"); });
		}
		if (!globalThis._singleFile_indexedDB) {
			globalThis._singleFile_indexedDB = globalThis.indexedDB;
			globalThis.__defineGetter__("indexedDB", () => { throw new Error("indexedDB temporary blocked by SingleFileZ"); });
		}
	});
